<template>
  <div
    class="ctaGrid grid grid-cols-1 md:grid-cols-2 max-w-lg mx-auto gap-6 py-8"
  >
    <nuxt-link
      :key="subheader"
      :to="link"
    ...
    >
      <div
        class="bg-primary h-0 bottom-0 group-hover:h-full absolute w-full transition-height ease-in duration-200"
      />
      <div class="px-6 py-8 h-full w-full relative">
        ...
        <div :class="`h-full flex flex-col ${!header && 'justify-center'}`">
          ...
          <span
            v-if="subheader"
            class="text-2xl text-primary group-hover:text-white transition ease-in duration-300 text-left block"
            >{{ subheader }}</span
          >
        </div>
      </div>
